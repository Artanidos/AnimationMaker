...
void PlayHead::paintEvent(QPaintEvent *)
{
    ...
    int offset = m_horizontalScrollPos * SCROLL_OFFSET;
    ...
    for(int i = 0 - offset; i < width; i+=100)
    {
        ...
        if(i > 0)
        {
            QString posString = Timeline::timeString((i + offset) * VALUE_OFFSET, false);
    ...
    painter.drawImage(m_value / VALUE_OFFSET - 4 - offset, 17, m_image);
}
...
void PlayHead::mousePressEvent(QMouseEvent *event)
{
    if(event->button() != Qt::LeftButton)
        return;
    if(event->x() + m_horizontalScrollPos * SCROLL_OFFSET > m_value / VALUE_OFFSET - 4 && event->x() + m_horizontalScrollPos * SCROLL_OFFSET <= m_value / 5 + 5)
        m_pressed = true;
    else
    {
        int x = event->x();
        ...
        if(x >= width())
            x = width() - 1;
        x += m_horizontalScrollPos * SCROLL_OFFSET;
        m_value = qRound((qreal)x * VALUE_OFFSET / 100) * 100;
        m_pressed = true;
        update();
